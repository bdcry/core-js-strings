...
function removeLastOccurrences(str, value) {
  const lastIdx = str.lastIndexOf(value);
  if (lastIdx < 0) return str;
  return str.slice(0, lastIdx) + str.slice(lastIdx + value.length);
}
...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, val) => val.charCodeAt() + acc, 0)
    : 0;
}
...
function isPalindrome(str) {
  ...
  let l = 0;
  let r = processedStr.length - 1;

  while (l < r) {
    if (processedStr[l] !== processedStr[r]) {
      return false;
    }
...
function findLongestWord(sentence) {
  ...
  sentenceArr.forEach((word, i) => {
    if (word.length > result.maxLen) {
      result.maxLen = word.length;
      result.idx = i;
    }
  });

  return sentenceArr[result.idx];
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < sentenceArr.length; i += 1) {
    res += ` ${sentenceArr[i].split('').reverse().join('')}`;
  }
...
function invertCase(str) {
  const invertedStr = str.split('').map((char) => {
